chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    if (request.action === 'startDownload') {
        ...
        // Start polling for progress (optional in background, mostly to keep SW alive if needed)
        ...
        .then((data) => {
            ...
            chrome.runtime.sendMessage({ action: 'downloadComplete', clientId: clientId }).catch(() => {});
        ...
        .catch((error) => {
            clearInterval(progressInterval);
            chrome.runtime.sendMessage({ action: 'downloadError', clientId: clientId, error: error.message }).catch(() => {});
            chrome.notifications.create({
                type: 'basic',
                iconUrl: 'icon.png',
